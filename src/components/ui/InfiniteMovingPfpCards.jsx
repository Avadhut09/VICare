...
export const InfiniteMovingPfpCards = ({
  items,
  direction = "left",
  speed = "normal",
  pauseOnHover = true,
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item) => (
          <li
            className="w-[350px] max-w-full relative rounded-2xl border border-b-0 flex-shrink-0 border-slate-500 px-8 py-6 md:w-[450px]"
            style={{
              background:
                "linear-gradient(180deg, var(--zinc-950), var(--zinc-800))",
            }}
            key={item.name}
          >
            <blockquote className="relative h-full flex flex-col justify-between">
              <div
                aria-hidden="true"
                className="overflow-y-auto user-select-none -z-1 pointer-events-none absolute -left-0.5 -top-0.5 h-[calc(100%_+_4px)] w-[calc(100%_+_4px)]"
              ></div>
